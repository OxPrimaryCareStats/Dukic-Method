...
fInvlogit <- function(x) { 1/(1+exp(-x)) }
...
fLoglikelihoodfixedalpha<-function(beta){ fLoglikelihood(c(0,beta)) } 	# Loglikelihood with alpha fixed at 0
...
fFpr<-function(theta, intercepthat) { 1-fInvlogit(theta/exp(intercepthat)) }

fThetatospec<-function(theta, intercept, alpha, beta) { fInvlogit(theta/exp(intercept)) }

fThetatosens<-function(theta, intercept, alpha, beta) { 1-fInvlogit( (theta-beta)/exp(intercept+alpha) ) }

fSpectotheta<-function(spec, intercept) { fLogit(spec)*exp(intercept) }
